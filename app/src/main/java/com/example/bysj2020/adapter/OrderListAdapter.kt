 ...
 *    Desc   : 订单列表适配器
 */
class OrderListAdapter(mList:List<OrderListRecord>,context: Context):BaseAdapter<OrderListRecord>(mList,context) {
    override fun covert(
        holder: BaseRecyclerHolder,
        mList: MutableList<OrderListRecord>,
        position: Int
    ) {
//        holder.setImageResource(R.id.item_orderList_img,mList[position].)
